...
st.title('Курсовой проект. Разработка макета веб-приложения.')
...
st.write('Данные отмасштабированы, целевой признак закодирован:')
st.write(data_clear)
...
data_clear_x = data_clear_x[cols_for_work]
st.write('Важные признаки отобраны:')      
st.write(data_clear_x)
st.write('Корреляционная матрица по всем важным признакам кроме целевого:')
fig, ax = plt.subplots(figsize=(20,15))
sns.heatmap(data_clear_x.corr(), ax=ax, annot=True, fmt='.3f')
st.pyplot(fig)
#data_vis1 = data
#data_vis1['Q'] = \
...
vine_X_train, vine_X_test, vine_y_train, vine_y_test = train_test_split(data_clear_x, data_clear_y, test_size = 0.5, random_state = 3)
